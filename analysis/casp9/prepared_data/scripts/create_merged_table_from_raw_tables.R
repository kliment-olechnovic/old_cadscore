...
casp9_additional_scores=read.table("../input_data/casp9_additional_scores_table", header=TRUE, stringsAsFactors=FALSE);
casp9_lga_sda_values=read.table("../input_data/casp9_LGA_sda_values", header=TRUE, stringsAsFactors=FALSE);

merged_table=merge(casp9_scorestable, casp9_additional_scores);
merged_table=merge(merged_table, casp9_lga_sda_values);
