...
for(second_score_name in second_score_names)
{
	...
	points(s1$min_RNA_INF, s3$subset_size/(s2$subset_size+s3$subset_size), type="l", col="red", lwd=3);
	points(s1$min_RNA_INF, s4$subset_size/s1$set_size, type="l", col="brown");
	points(s1$min_RNA_INF, s1$set_size/max(s1$set_size), type="l", col="black");
	legend(0, -0.35, c("Filtered by minimum RNA_INF in a pair", "Both agreed with MP", "CAD-score agreed with MP", "CAD-score disagreed with MP", "Both disagreed with MP"), lwd=c(1, 1, 3, 3, 1), col=c("black", "blue", "green", "red", "brown"));
}
