...
$CADSCORE_BIN_DIR/voroprot2 --mode x-chop-chain-ends --output-prefix "$RENUMBERED_MODEL_FILE.chopped" < $RENUMBERED_MODEL_FILE

for i in $(seq 0 3 24)
do
  $CADSCORE_BIN_DIR/CADscore_calc.bash -D $OUTPUT_DIR/db -t $TARGET_FILE -m "$RENUMBERED_MODEL_FILE.chopped_$i" -a -n -u -q -e "rnadi.bash" -y -g -x
done
