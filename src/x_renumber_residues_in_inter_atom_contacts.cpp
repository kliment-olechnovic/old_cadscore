...
std::string collect_sequence_string_from_residue_ids_map(const T& residue_ids_map)
{
	...
	for(typename T::const_iterator it=residue_ids_map.begin();it!=residue_ids_map.end();++it)
	{
		output << protein::LettersCoding::convert_residue_codes_big_to_small(it->second.name);
	}
	return output.str();
}
...
void x_renumber_residues_in_inter_atom_contacts(const auxiliaries::CommandLineOptions& clo)
{
	...
	for(std::size_t l=0;l<chain_names.size();l++)
	{
		...
		alignments[0]=protein::PairwiseSequenceAlignment::construct_global_sequence_alignment(sequence_string_from_atoms_1, sequence_string_from_input_alignment_1, scorer);
		alignments[1]=protein::PairwiseSequenceAlignment::read_sequence_alignment(input_alignment_string_1, input_alignment_string_2);
		alignments[2]=protein::PairwiseSequenceAlignment::construct_global_sequence_alignment(sequence_string_from_input_alignment_2, sequence_string_from_atoms_2, scorer);
