...
QuadruplesMap find_surplus_valid_quadruples(const BoundingSpheresHierarchy<SphereType>& bsh, const QuadruplesMap& quadruples_map, QuadruplesLog& quadruples_log)
{
					...
					for(std::size_t b=a+1;b<refined_collisions.size();b++)
					{
						for(std::size_t c=b+1;c<refined_collisions.size();c++)
						{
							for(std::size_t d=c+1;d<refined_collisions.size();d++)
							{
								...
								{
									quadruples_log.surplus_quadruples++;
									quadruples_log.surplus_tangent_spheres++;
