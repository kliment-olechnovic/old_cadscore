...
template<typename OutputPointType, typename InputSphereTypeA, typename InputSphereTypeB, typename InputSphereTypeC>
inline std::vector<OutputPointType> construct_spheres_tangent_planes_normals_safe(const InputSphereTypeA& sm, const InputSphereTypeB& s1, const InputSphereTypeC& s2)
{
	{
		const double min_r=std::min(sm.r, std::min(s1.r, s2.r));
		if(sm.r!=min_r)
...
template<typename InputSphereTypeA, typename InputSphereTypeB, typename InputSphereTypeC>
inline std::vector< std::pair<SimplePoint, SimplePoint> > construct_spheres_tangent_planes_safe(const InputSphereTypeA& a, const InputSphereTypeB& b, const InputSphereTypeC& c)
{
	const std::vector<SimplePoint> normals=construct_spheres_tangent_planes_normals_safe<SimplePoint>(a, b, c);
	std::vector< std::pair<SimplePoint, SimplePoint> > planes;
	for(std::size_t i=0;i<normals.size();i++)
	{
		planes.push_back(std::make_pair((custom_point_from_object<SimplePoint>(a)+(normals[i]*a.r)), normals[i]));
