...
{
	std::map< Key, std::pair<Value1, Value2> > combined_map;
	...
	while(it_1!=map_1.end() || it_1!=map_2.end())
	{
		...
		{
			combined_map[it_1->first]=std::make_pair(it_1->second, it_2->second);
			++it_1;
			++it_2;
		}
		else if(it_1!=map_1.end() && (it_2==map_2.end() || it_1->first<it_2->first))
		{
			combined_map[it_1->first]=std::make_pair(it_1->second, Value2());
			++it_1;
		}
		else if(it_2!=map_2.end() && (it_1==map_1.end() || it_2->first<it_1->first))
		{
			combined_map[it_2->first]=std::make_pair(Value1(), it_2->second);
			++it_2;
		}
	}
	return combined_map;
