...
std::map<ResidueID, ResidueContactAreaDifferenceScore> construct_residue_contact_area_difference_profile(
		...
		const std::set<ResidueID>& residue_ids)
{
	std::map<ResidueID, ResidueContactAreaDifferenceScore> profile;
	{
		...
		for(typename std::set<ResidueID>::const_iterator it=residue_ids.begin();it!=residue_ids.end();++it)
		{
			prev=profile.insert(prev, std::make_pair(*it, ResidueContactAreaDifferenceScore()));
		}
	}
