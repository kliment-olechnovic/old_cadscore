...
USE_TMSCORE="$SCRIPT_DIRECTORY/use_tmscore.bash"
...
if [ ! -f "$USE_TMSCORE" ]
then
  echo "$USE_TMSCORE does not exist" 1>&2
  exit 1
fi
...
test -f $TMSCORE_GLOBAL_SCORES_FILE || $USE_TMSCORE -m $MODEL_FILE -t $TARGET_FILE -p $TMSCORE_PROFILE_FILE -s $TMSCORE_GLOBAL_SCORES_FILE
